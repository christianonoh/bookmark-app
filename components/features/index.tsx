...
const FeaturesSection = () => {
  ...
  return (
    <section className="flex flex-col py-16 items-center">
      ...
      <div className="flex flex-col md:flex-row px-5 mb-16 w-full max-w-3xl mx-auto">
        <div className="border-b border-gray-light w-full md:hidden" />
        {features.map((feature, index) => (
          <div
            key={index}
            className={cx(
              "flex-1 text-center px-12  py-4 md:py-6 cursor-pointer whitespace-nowrap hover:text-red transition-all duration-200 ease-in-out",
              selectedFeature === feature.key
                ? "border-b-4 border-red text-dark font-medium"
      ...
      {features.map((feature, index) => (
        <div
          ...
          className={cx(
            "w-full",
            selectedFeature === feature.key ? "block" : "hidden"
          )}
        >
          <Feature
            img={feature.img}
