...
const FeaturesSection = () => {
  ...
  return (
    <section className='flex flex-col py-16 items-center'>
      <h2 className="text-4xl capitalized font-bold mb-8">Features</h2>
    ...
    <div className="flex flex-col md:flex-row px-5 mb-16 w-full">
      <div className="border-b border-gray-light w-full md:hidden" />
      {
        features.map((feature, index) => (
          <div key={index} className={cx('flex-1 text-center px-12  py-6 cursor-pointer whitespace-nowrap', selectedFeature === feature.key ? 'border-b-4 border-red text-dark font-bold' : 'text-gray-medium border-b border-gray-light')} onClick={() => handleClick(feature.key)}>
            {feature.title}
          </div>
        ))
      }
